# ADR 260914: Kotoba Cloud research inference connects directly to Modal
...
Modal deployment directly. Its configuration is:

- `MODAL_INFERENCE_URL`: an HTTPS `.modal.run` OpenAI-compatible completion
  endpoint.
- `MODAL_INFERENCE_TOKEN`: its server-only bearer token.

The authority rejects missing configuration, a non-Modal host, failed upstream
...
run, operators must configure the exact Modal deployment URL and token in the
...
direct Modal URL, bearer-header, model-attribution, admission, replay, and
