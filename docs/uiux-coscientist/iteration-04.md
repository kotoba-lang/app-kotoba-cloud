# shinkansen Co-Scientist Iteration 04 — UI/UX kaizen

> Seed: kotoba.cloud emitted documents under public
>
> Judge: `shinkansen.audit` (deterministic, no LLM, no browser) over 104 emitted document(s). Overall **99.2 / 100**.
...
|---|---|---|---|---|---|---|
| 1 | `sk-h1` | consumer | S | 1246 | +25.5 | emit locale-free hrefs (/#…, /billing/) and let the edge negotiate by cookie (shinkansen.locale) |
| 2 | `sk-h2` | consumer | S | 1215 | +22.8 | keep one sentence beside each action; move explanation under <details> or link the guide |
| 3 | `sk-h3` | consumer | S | 1185 | +17.5 | one page-level action for refresh/retry; per-panel repeats are removed |
| 4 | `sk-h4` | consumer | S | 1154 | +16.1 | hide a control until its state is known, or enable it with the reason beside it — never a bare disabled button in the idle document |

## Batch shipped this iteration

`sk-kaizen-batch` = `sk-h1`, `sk-h2`, `sk-h3`, `sk-h4` — projected 99.2 → **100 / 100**. Deferred: none.
...
## Delta from iteration 03 (the measurement)

overall 92.8 → **99.2** (+6.5)
...
| `idle-disabled` | 0.964 | 0.965 |
| `repeated-actions` | 0.961 | 0.962 |
| `plain-labels` | 1.000 | 1.000 |
| `note-density` | 0.939 | 0.940 |
| `locale-path-links` | 0.931 | 0.933 |
| `fixed-anchor` | 1.000 | 1.000 |
| `links-resolve` | 0.265 | 1.000 |
| `csp-allows-assets` | 1.000 | 1.000 |
| `pre-overflow` | 1.000 | 1.000 |

closed: links-resolve
opened: none
