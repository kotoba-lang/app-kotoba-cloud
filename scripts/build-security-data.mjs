...
const root={version:1,schema:schemaLink,query:queryLink,licenses,corpus,generatedAt:lock.kev.retrievedAt,index:axes,coverage:{kev:{selected:50,total:kev.vulnerabilities.length,selection:'latest dateAdded descending then CVE id'},attack:{groups:groups.length,techniques:techniques.length,relations:relations.length-1,selection:'all non-revoked, non-deprecated Enterprise ATT&CK intrusion sets with MITRE IDs; reported uses edges to valid techniques only',language:'source labels and aliases retained; no translation',geography:'source-covered activity clusters; no inferred nationality or exhaustive global coverage'},logs:'one public OTRF lab audit dataset; no private incident logs',scap:'overview and specification catalog; no SCAP evaluation engine'},sources:[...sourceLinks.values()],items:records.map(x=>x.record),claims:claimLinks,ontology:ontologyLink,linkedData:graphLink,datoms:datomsLink,scenario:scenarioLink};const head=dag(root);
